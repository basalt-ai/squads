...
`outreach-agent` wakes on a daily heartbeat. It checks what's due in the active pipeline (follow-ups, pending replies), finds new leads if the daily quota hasn't been hit, executes the next touch on each active sequence, handles any replies that came in, logs one weekly learning if it's the last heartbeat of the week, and always closes with a digest posted to your chosen channel. The user never needs to talk to the agent directly — route requests through your Pancake co-founder.
